...
- **自动地址宽度**：W25Q16~W25Q128 使用 24 位地址；W25Q256/W25Q512 自动切换 4 字节地址扩展命令（读 0x13 / 写 0x12 / 擦除 0x21/0xDC；32K 块擦除回退为 8 次 4K 扇区擦除 0x21，因 W25Q256JV 无 32K 块擦除的 4 字节地址专用命令），**无需修改芯片模式寄存器**，掉电唤醒后无需重新配置。
